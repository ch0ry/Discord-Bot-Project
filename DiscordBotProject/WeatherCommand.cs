...
namespace DiscordBotProject.Commands
{
    class WeatherCommand : BaseCommandModule
    {
        ...
        [Command("manrique")]
        public async Task Manrique(CommandContext ctx)
        {

            if (ctx.Member.Nickname == "Andres Manrique")
            {
                await ctx.Channel.SendMessageAsync("Gei").ConfigureAwait(false);

            }
            else
            {
                await ctx.Channel.SendMessageAsync("God").ConfigureAwait(false);
            }
        }
